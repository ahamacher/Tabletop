...
router.post("/image/:image_id", (req, res) => {
    Image.findById(req.params.image_id)
        .then(image => {
            ...
            const imageInstanceParams = {};
            imageInstanceParams.image_id = image.id;
            
            
            if (!isNaN(req.body.positionX) && !isNaN(req.body.positionY)) {
                imageInstanceParams.positionX = req.body.positionX,
                imageInstanceParams.positionY = req.body.positionY
            }

            const newImageInstance = new ImageInstance(imageInstanceParams)
